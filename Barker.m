...
peak_power = radareqpow(lambda,max_range,snr_min,NumChips/bw,...
...
% redefine Targets
tgtpos = [[-1600 0 400]',[3000 0 0]',[9000 0 0]'];
tgtvel = [[100 100 0]',[-100 0 0]',[200 0 0]'];
tgtmotion = phased.Platform('InitialPosition',tgtpos,'Velocity',tgtvel);
tgtrcs = [2.2 20.1 15.05];
...
matchedfilter = phased.MatchedFilter('Coefficients',matchingcoeff,...
    'CoefficientsSource','Property','GainOutputPort',true);
...
for i = 1:num_360_rotations
    ...
    for j = 1:numscans
        ...
        rxPulse = rxpulses;
        [mfiltOut,mfgain] = matchedfilter(rxPulse);

        matchingdelay = size(matchingcoeff,1)-1;
        mfiltOut = buffer(mfiltOut(matchingdelay+1:end),size(mfiltOut,1));
        h = [1 -2 1];
        mtiseq = filter(h,1,tvg(mfiltOut),[],2);
    ...
    threshold = threshold * db2pow(mfgain+6)*13;
    
    [~,peakInd] = findpeaks(int_pulses(1:end),'MinPeakHeight',threshold);
...
npower = noisepow(noise_bw,receiver.NoiseFigure,...
    receiver.ReferenceTemperature);
threshold = npower * db2pow(npwgnthresh(pfa,num_pulse_int,'noncoherent'));
threshold = threshold * db2pow(mfgain+6)*6;
% Plot the received pulse energy again range
plot(range_gates,pow2db(abs(screens1(:,1,1)).^2),'b-'...
    ,range_gates,pow2db(abs(threshold*ones(1,104)))); grid on;
